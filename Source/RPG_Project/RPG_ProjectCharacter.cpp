...
void ARPG_ProjectCharacter::BeginPlay() {
	Super::BeginPlay();
	playerJob = NewObject<UJob>(this);
	logJobName();

	MyMode = Cast<ARPG_ProjectGameMode>(UGameplayStatics::GetGameMode(GetWorld()));
}

void ARPG_ProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("TurnRate", this, &ARPG_ProjectCharacter::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &ARPG_ProjectCharacter::LookUpAtRate);

	// handle touch devices
	PlayerInputComponent->BindTouch(IE_Pressed, this, &ARPG_ProjectCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &ARPG_ProjectCharacter::TouchStopped);

	// VR headset functionality
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &ARPG_ProjectCharacter::OnResetVR);
}

void ARPG_ProjectCharacter::OnRun() {
	isRun = true;
}

void ARPG_ProjectCharacter::OnWalk() {
	isRun = false;
}
...
{
	// If RPG_Project is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in RPG_Project.Build.cs is not automatically propagated
...
void ARPG_ProjectCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void ARPG_ProjectCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}

void ARPG_ProjectCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ARPG_ProjectCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
...
void ARPG_ProjectCharacter::setJob(JobName job) {
	switch (job)
	{
